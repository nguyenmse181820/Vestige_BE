...
-- Fixed version that avoids DISTINCT in window functions and aggregates
...
-- Step 1: Generate base slugs for all products
WITH base_slugs AS (
    SELECT 
        p.product_id,
        CASE 
...
-- Step 2: Get unique sellers per base slug (avoiding COUNT DISTINCT)
unique_sellers AS (
    SELECT DISTINCT base_slug, seller_id
    FROM base_slugs
),
seller_counts AS (
    SELECT 
...
-- Step 3: Get the first seller for each base slug
first_products AS (
    SELECT 
        base_slug,
...
first_sellers AS (
    ...
    FROM base_slugs bs
    INNER JOIN first_products fp ON bs.base_slug = fp.base_slug AND bs.product_id = fp.first_product_id
),
-- Step 4: Final analysis with row numbers
slug_analysis AS (
    SELECT 
        bs.product_id,
        bs.base_slug,
...
-- Step 5: Update products with final slugs
UPDATE products 
SET slug = (
    SELECT 
        CASE 
