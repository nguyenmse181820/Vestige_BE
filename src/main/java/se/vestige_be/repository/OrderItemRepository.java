...
public interface OrderItemRepository extends JpaRepository<OrderItem, Long> {
    ...
    @Query("SELECT oi FROM OrderItem oi WHERE oi.seller.userId = :sellerId ORDER BY oi.order.createdAt DESC")
    ...
    @Query("SELECT oi FROM OrderItem oi WHERE oi.seller.userId = :sellerId AND oi.status = :status ORDER BY oi.order.createdAt DESC")
